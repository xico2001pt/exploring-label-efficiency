...
class SelfSLTrainer(Trainer):
    def __init__(self, model, device, logger, method):
    ...
    def _epoch_iteration(self, dataloader, is_train=True, optimizer=None, metrics={}, description="Train"):
        ...
        with torch.set_grad_enabled(is_train):
            self._selfsl_batch_iteration(self.num_batches, dataloader, optimizer, metrics, total_loss, loss_counter, total_metrics, description)

        for loss_term in total_loss:
            total_loss[loss_term] /= self.num_batches

        for metric in total_metrics:
            total_metrics[metric] /= self.num_batches
    ...
    def train(
        ...
        self.num_batches = min(len(train_dataloader), max_num_batches)
